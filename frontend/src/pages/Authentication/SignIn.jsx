...
const SignIn = () => {
  ...
  // WE NEED TO CHANGE CONDITION BASED ON POST API AND STORE JWT TOKEN
  // IF THE LOGIN IS SUCCESSFULL THEN I WILL GET USER TOKEN WHICH I NEED TO STORE IT IN LOCAL STORAGE

  const handleLogin = (e) => {
    // CALL API BY SENDING USER DATA - POST AS WELL AS DISPATCH USER DETAILS & TOKEN TO REDUX & STORE TOKEN TO LOCAL STORAGE
    // IF THE USER IS VERIFIED WE WILL GET USER TICKET
    e.preventDefault();
    if (
      isAuthenticated &&
      ...
      dispatch(setToken(info.user.role));
      navigate('/dashboard');
              ...
              <div className="relative flex mt-4 w-full rounded-lg border-gray-500 p-4 pe-12 text-sm bg-[#fff] shadow-lg">
                <label className="mr-4 flex items-center text-xs">
                  ...
                  <input
                    ...
                    checked={role === 'admin'}
                    onChange={() =>
                      dispatch(login({ ...info.user, role: 'admin' }))
                    }
                ...
                <label className=" flex items-center text-xs">
                  <input
                    ...
                    checked={role === 'user'}
                    onChange={() =>
                      dispatch(login({ ...info.user, role: 'user' }))
                    }
